    ...
    button.on('click', function(event){
        ...
        makeRequest(`file/${fileId}/access/deprive`, 'patch', data).done(
            function(status, message, data){
                console.log('Successfully deprived access from poor man!');
                console.log(status);
                console.log(message);
...
function addUserLink(data){
    ...
    $(`<div class="btn-group btn-group-sm w-50">
            <a class='btn btn-info' href="http://localhost:8000/accounts/profile/${data.id}/"><i class="far fa-eye"></i> ${ data.username }</a>
